export enum Position { LEFT = 1, CENTER, RIGHT }
...
export default class Slate {
    ...
    private static toLongestMessage(longestMessage: string, message: string): string {
    ...
    private static stackMessage(maxLength: number, position: Position) {
        return function toStackedMessage(stackedMessage: string, message: string): string {
